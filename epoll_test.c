...
static uint8_t *p_buffer_array;/* Actual cf_vallocd p_buffers */

int eventfd_array[MAX_EVENTFDS + 10];
int start_efd, current_efd, last_efd;
pthread_mutex_t efd_lock;
...
static void* timer_func(void *async_epfd)
{
	uint64_t count = 0;
	int sleep_ms, epfd = *(int*)async_epfd;
...
//TODO do you set buffer to 0 or does it have values? 
	memset(p_buffer, 0, sizeof(p_readreq->size));
...
static void create_eventfd_array(int async_epfd)
{
...
//	create_eventfd_queue(async_epfd);
	create_eventfd_array(async_epfd);
	...
	pthread_t timer;
	if (pthread_create(&timer, NULL, &timer_func , &async_epfd)) 
	{
		fprintf(stdout, "Error: creating timer thread failed\n");
		exit(-1);
	}	
